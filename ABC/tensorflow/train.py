...
class Trainer(object):
    def __init__(self,
        ...
        self.loss_objects = [
            YoloLoss(num_classes=TOTAL_CLASSES,
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            for loss_object, y_pred, y_true in zip(self.loss_objects, outputs,
                                                   labels):
                total_loss, loss_breakdown = loss_object(y_true,
                                   y_pred)
    ...
    def run(self, train_dist_dataset, val_dist_dataset):
        ...
        def distributed_train_epoch(dataset):
            ...
            for one_batch in dataset:
                ...
                batch_xy_loss = self.strategy.reduce(
                    tf.distribute.ReduceOp.SUM, per_replica_xy_losses, axis=None)
                batch_wh_loss = self.strategy.reduce(
                    tf.distribute.ReduceOp.SUM, per_replica_wh_losses, axis=None)
                batch_class_loss = self.strategy.reduce(
                    tf.distribute.ReduceOp.SUM, per_replica_class_losses, axis=None)
                batch_obj_loss = self.strategy.reduce(
                    tf.distribute.ReduceOp.SUM, per_replica_obj_losses, axis=None)
                total_loss += batch_loss
                num_train_batches += 1
                t1 = time.time()
                tf.print('Trained batch:', num_train_batches, 'batch loss:',
                         batch_loss, 'batch xy loss', batch_xy_loss, 'batch wh loss', batch_wh_loss, 'batch obj loss', batch_obj_loss, 'batch_class_loss', batch_class_loss, 'epoch total loss:', total_loss, 'duration seconds:', round(t1-t0, 3), 'examples/seconds:', self.global_batch_size / (t1-t0))
        ...
        print('{} Start training...'.format(datetime.now().strftime("%m/%d/%Y %H:%M:%S")))
        for epoch in range(1, self.epochs+1):
            t0 = time.time()
            self.lr_decay()

            print('{}   Started epoch {} with learning rate {}. Current LR patience count is {} epochs. Last lowest val loss is {}.'.format(
                datetime.now().strftime("%m/%d/%Y %H:%M:%S"), epoch, self.current_learning_rate, self.patience_count, self.lowest_val_loss))

            train_total_loss, num_train_batches = distributed_train_epoch(
                train_dist_dataset)
            t1 = time.time()
            train_loss = train_total_loss / num_train_batches
            print('{}   Epoch {} train loss {}, total train batches {}, {} examples per second'.format(
                datetime.now().strftime("%m/%d/%Y %H:%M:%S"), epoch, train_loss, num_train_batches, num_train_batches*self.global_batch_size/(t1-t0)))
            ...
            print('{}   Epoch {} val loss {}, total val batches {}, {} examples per second'.format(
                datetime.now().strftime("%m/%d/%Y %H:%M:%S"), epoch, val_loss, num_val_batches, num_val_batches*self.global_batch_size/(t2-t1)))
        ...
        print('{} Finished.'.format(datetime.now().strftime("%m/%d/%Y %H:%M:%S")))

    def save_model(self, epoch, loss):
        # https://github.com/tensorflow/tensorflow/issues/33565
        self.model.save_weights('./models/model-v1.0.8-epoch-{}-loss-{:.4f}.tf'.format(epoch, loss))
        print("Model saved")
...
def main():
    ...
    with strategy.scope():
        ...
        val_dist_dataset = strategy.experimental_distribute_dataset(
            val_dataset)
        model = YoloV3(shape=(416, 416, 3), num_classes=TOTAL_CLASSES, training=True)
        model.summary()
