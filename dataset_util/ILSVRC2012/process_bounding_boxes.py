...
if __name__ == '__main__':
    ...
    print(
        'Identified %d XML files in %s' % (len(xml_files), sys.argv[1]),
        file=sys.stderr)
    ...
    for file_index, one_file in enumerate(xml_files):
        ...
        for bbox in bboxes:
            ...
            if (bbox.xmin_scaled >= bbox.xmax_scaled
            ...
            print('%s.JPEG,%.4f,%.4f,%.4f,%.4f' %
                  (image_filename, bbox.xmin_scaled, bbox.ymin_scaled,
        ...
        if not file_index % 5000:
            print(
                '--> processed %d of %d XML files.' % (file_index + 1,
                                                       len(xml_files)),
                file=sys.stderr)
            print(
                '--> skipped %d boxes and %d XML files.' % (skipped_boxes,
                                                            skipped_files),
                file=sys.stderr)

    print(
        'Finished processing %d XML files.' % len(xml_files), file=sys.stderr)
    print(
    ...
    print(
        ...
        file=sys.stderr)
    print(
        'Wrote %d bounding boxes from %d annotated images.' % (saved_boxes,
                                                               saved_files),
        file=sys.stderr)
