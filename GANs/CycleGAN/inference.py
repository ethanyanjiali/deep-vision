...
def main():
    ...
    def generate(filename, idx, kind):
        ...
        im1.save('samples_{}/male2female_{}_{}_original.JPEG'.format(ds, kind, idx))
        generated = tf.image.resize(generated, (218, 178))
        generated = tf.cast(generated, tf.uint8)
        im2 = Image.fromarray(generated.numpy())
        im2.save('samples_{}/male2female_{}_{}_generated.JPEG'.format(ds, kind, idx))

    files = glob.glob(os.path.join('datasets', ds, 'trainA', '*'))
    # generate('/Users/yanjia.li/Downloads/img_align_celeba/000030.jpg', 30, 'male2female')
    for idx in range(20):
        generate(files[idx], idx, 'celeba')
